defmodule Sagents.SubAgentTest do
  ...
  alias Sagents.SubAgent.Config, as: SubAgentConfig
  alias Sagents.SubAgent.Compiled, as: SubAgentCompiled
  ...
  describe "SubAgentCompiled.new/1" do
    ...
    test "validates agent must be Agent struct" do
      ...
      assert {:error, changeset} = SubAgentCompiled.new(attrs)
      assert %{agent: ["must be a Sagents.Agent struct"]} = errors_on(changeset)
    end
  ...
  describe "subagent_middleware_stack/2" do
    ...
    test "filters out SubAgent middleware without config tuple" do
      ...
      assert length(result) == 2
      refute Enum.any?(result, &match?(Sagents.Middleware.SubAgent, &1))
    end
    ...
    test "filters out SubAgent middleware from mixed formats" do
      ...
      middleware_entry = %MiddlewareEntry{
        id: Sagents.Middleware.SubAgent,
        module: Sagents.Middleware.SubAgent,
        config: %{}
      }
  ...
  describe "subagent_middleware_stack/3 with block_middleware option" do
    alias Sagents.Middleware.SubAgent, as: SubAgentMW
  ...
  describe "is_subagent_middleware?/1" do
    test "returns true for raw SubAgent module" do
      assert SubAgent.is_subagent_middleware?(Sagents.Middleware.SubAgent)
    end

    test "returns true for SubAgent tuple" do
      assert SubAgent.is_subagent_middleware?({Sagents.Middleware.SubAgent, []})
      assert SubAgent.is_subagent_middleware?({Sagents.Middleware.SubAgent, [opt: 1]})
    end
    ...
    test "returns false for other middleware modules" do
      refute SubAgent.is_subagent_middleware?(Sagents.Middleware.TodoList)
      refute SubAgent.is_subagent_middleware?({Sagents.Middleware.TodoList, []})
    end

    test "returns false for other MiddlewareEntry structs" do
      entry = %MiddlewareEntry{
        id: Sagents.Middleware.TodoList,
        module: Sagents.Middleware.TodoList,
        config: %{}
      }
  ...
  describe "extract_middleware_module/1" do
    ...
    test "extracts module from MiddlewareEntry struct" do
      ...
      assert SubAgent.extract_middleware_module(entry) == Sagents.Middleware.TodoList
    end
