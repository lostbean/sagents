defmodule Sagents.AgentDeltaOrderingTest do
  ...
  # ---------------------------------------------------------------------------
  # Group 3: clear_interrupt_from_tool_results/1
  # ---------------------------------------------------------------------------

  describe "clear_interrupt_from_tool_results/1" do
    test "clears interrupt_data from State in processed_content" do
      state_with_interrupt =
        State.new!(%{
          metadata: %{key: "preserved"},
          interrupt_data: %{type: :hitl, tool: "my_tool"}
        })
      ...
      result =
        ToolResult.new!(%{
          ...
          name: "my_tool",
          content: "ok",
          processed_content: state_with_interrupt
        })

      msg = Message.new_tool_result!(%{tool_results: [result]})
      cleaned = Agent.clear_interrupt_from_tool_results(msg)

      [cleaned_result] = cleaned.tool_results
      assert cleaned_result.processed_content.interrupt_data == nil
      assert cleaned_result.processed_content.metadata == %{key: "preserved"}
    end

    test "leaves non-State processed_content unchanged" do
      result =
        ToolResult.new!(%{
          tool_call_id: "call_1",
          name: "plain_tool",
          content: "result text",
          processed_content: "just a string"
        })
      ...
      [cleaned_result] = cleaned.tool_results
      assert cleaned_result.processed_content == "just a string"
    end

    test "handles nil tool_results" do
      msg = Message.new_user!("hello")
      # User messages have nil tool_results, but we can also build a tool-like message
      # The function pattern matches on %Message{tool_results: nil}
      tool_msg = %Message{msg | tool_results: nil}
      assert Agent.clear_interrupt_from_tool_results(tool_msg) == tool_msg
    end

    test "only clears results that have interrupt_data" do
      state_with_interrupt =
        State.new!(%{
          metadata: %{tool: "a"},
      ...
      state_without_interrupt = State.new!(%{metadata: %{tool: "b"}})
      ...
      result_b =
        ToolResult.new!(%{
          ...
          name: "tool_b",
          content: "ok",
          processed_content: state_without_interrupt
        })

      msg = Message.new_tool_result!(%{tool_results: [result_a, result_b]})
      cleaned = Agent.clear_interrupt_from_tool_results(msg)

      [cleaned_a, cleaned_b] = cleaned.tool_results
      # A had interrupt_data, should be cleared
      assert cleaned_a.processed_content.interrupt_data == nil
      assert cleaned_a.processed_content.metadata == %{tool: "a"}
      # B had no interrupt_data, should be unchanged
      assert cleaned_b.processed_content.interrupt_data == nil
      assert cleaned_b.processed_content.metadata == %{tool: "b"}
    end
  end

  # ---------------------------------------------------------------------------
  # Group 4: update_chain_state_from_tools/1
  # ---------------------------------------------------------------------------
